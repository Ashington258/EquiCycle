...
import requests
import threading
import matplotlib.pyplot as plt  # 用于绘制帧率曲线
...
class Config:
    ...
    INPUT_SOURCE = "2.mp4"  # 视频路径、摄像头ID或URL
    CONF_THRESH = 0.25
    IMG_SIZE = 1280
...
def main():
    ...
    print(f"平均帧率: {average_fps:.2f}")

    # 绘制FPS曲线
    plt.figure(figsize=(10, 5))
    plt.plot(fps_list, label="FPS")
    plt.axhline(
        average_fps, color="r", linestyle="--", label=f"Average FPS: {average_fps:.2f}"
    )
    plt.xlabel("Frame")
    plt.ylabel("FPS")
