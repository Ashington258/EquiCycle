...
class Config:
    ...
    R = 500  # 调节舵机力度的参数，越大舵机力度越小
    SERVO_MIDPOINT = 1070  # 舵机中值脉冲宽度
    ALPAH = 0.3  # 平滑系数，范围在 0-1，数值越小平滑程度越高

    # 目标位置参数
    HORIZONTAL_LINE_Y = 280  # 横线的Y坐标
    TARGET_X = 306  # 目标的 X 坐标（摄像头中值）
    ...
    ZEBRA_OR_TURN_CONFIRMATION_DURATION = 0.1  # 持续监测时间
    ZEBRA_OR_TURN_IDLE_TIME = 1  # 持续IDLE状态时间
    # 转向标志类
    TURN_SIGN_CT = 0.75  # 转向标志的置信度

    # 任务类参数
    # 停车和变道类任务
    STABILIZATION_TIME = 2  # 停车后等待车身稳定时间
    LANE_CHANGE_ANGLE = 70  # 变道角度
    LANE_CHANGE_SPEED = -4  # 变道速度
