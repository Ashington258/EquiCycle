...
class CascadedPIDController:
    ...
    def update(self, current_angular_velocity, current_angle, current_velocity):
        ...
        if self.gyro_update_counter % 2 == 0:
            self.angle_pid.setpoint = self.velocity_control_signal # 将速度环的输出设为角度环的目标
            self.angle_control_signal = self.angle_pid.update(current_angle-offset_angle)

        # 更新角速度环，每个周期计算四次
        self.gyro_pid.setpoint = self.angle_control_signal  # 将角度环的输出设为角速度环的目标
...
def process_ch100_data(data):
    ...
    angular_velocity = data.get("gyr", [0.0, 0.0, 0.0])  # Assuming 'gyr' contains angular velocity data
