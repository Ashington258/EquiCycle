            ...
            if result.masks is not None:
                ...
                for i in range(result.masks.data.shape[0]):
                    if i == class_id:  # 如果当前掩膜的类ID与检测到的类ID匹配
                        ...
                        # 骨架化处理
                        binary_mask = mask_resized > 0  # 转换为二进制掩膜
                        skeleton = skeletonize(binary_mask).astype(np.uint8) * 255

                        # 创建骨架颜色图
                        skeleton_color = np.zeros_like(frame)
                        skeleton_color[skeleton > 0] = (0, 0, 255)  # 红色骨架

                        # 叠加掩膜和骨架到帧上
                        frame = cv2.addWeighted(frame, 1, color_mask, 0.5, 0)
                        frame = cv2.addWeighted(frame, 1, skeleton_color, 1, 0)
