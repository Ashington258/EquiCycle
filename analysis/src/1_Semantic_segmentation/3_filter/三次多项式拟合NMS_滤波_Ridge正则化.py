...
def apply_nms(results, iou_threshold=0.45):
    """应用NMS过滤边界框和分割掩膜"""
...
def fit_lane_points_and_draw(frame, skeleton, color=(0, 0, 255)):
    """对骨架点拟合正则化多项式并绘制拟合曲线，去除离群点并增加抗突变能力"""
    # 获取骨架点的坐标
    ...
    degree = 3  # 三次多项式
    model = make_pipeline(
        PolynomialFeatures(degree), Ridge(alpha=1e-2)
    )  # alpha控制正则化强度
    model.fit(y_coords.reshape(-1, 1), x_coords)  # 拟合数据
    y_range = np.linspace(min(y_coords), max(y_coords), num=500).astype(int)
    x_fitted = model.predict(y_range.reshape(-1, 1)).astype(int)
